...
AppDataSource.initialize()
  .then(async () => {
    ...
    app.get("/eficientize", (req, res) => {
      let feed = getFeed();
      feed.then((feed) => {
        res.render("eficientize.hbs", {
          captcha: recaptcha.render(),
          login: req.session.login,
          user: req.session.user,
          adm: req.session.administrador,
          rss: feed,
          instagram: process.env.INS_EFI,
          facebook: process.env.FAC_EFI,
          linkedin: process.env.LIN_PRI,
        });
      });
    });
    ...
    app.get(
      "/validarUsuario/:secret/:newAcount",
              ...
              if (acValidador instanceof AcountValidator) {
                ...
                if (acValidador.newAcount) {
                  emailController.enviar(
                    acValidador.email,
                    acValidador.parameter,
                    acValidador.newAcount,
                  );
                } else {
                  emailController.enviar(
                    acValidador.newEmail,
                    acValidador.parameter,
                    acValidador.newAcount,
                  );
                }
